...
class CategoriesService:
    ...
    model = Category  # Define the model to be used in the service
    ...
    def get_category(self, category_id: uuid.uuid4) -> Category:
        ...
        try:
            ...
            raise ValueError("Category not found")  # Custom error handling
    ...
    def get_subcategories(self, parent_id: uuid.uuid4, values: tuple[str]) -> list[Category] | list[dict]:
        """Retrieve subcategories for a given parent category, with optional filtering."""
        parent = self.get_category(parent_id)
        if values:
